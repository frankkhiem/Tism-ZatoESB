...
class LoadOutlookContacts(Service):
  ...
  def handle(self):
    ...
    if res_load_contacts['status_code'] == 200 :
      ...
      outlookAccount.email = self.getEmailAccount(outlookAccount.access_token)
      user.save()
      response.payload = {
        'success': True,
  ...
  def getEmailAccount(self, accessToken):
    get_email_conn = self.outgoing.plain_http['Load Outlook Account'].conn
    ...
    return res_get_email.json()['userPrincipalName']
